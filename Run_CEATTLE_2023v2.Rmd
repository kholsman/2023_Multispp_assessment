   ...
   if(1==10) source("R/get_EBSTemp.R")
    ...
    # make the ROMSNPZ indices
      ...
      if(!dir.exists(thisyr_fl))
        ...
        download.file(url ="https://zenodo.org/record/4586950/files/Bering10K_simulation_variables.xlsx?download=1",destfile=file.path(thisyr_fl,"Bering10K_simulation_variables.xlsx"))
var_def <- readxl::read_xlsx(file.path(thisyr_fl,"Bering10K_simulation_variables.xlsx"),
                             col_names = TRUE,skip=2)
oxygen_thresh <- data.frame(mg_l = "<3",category = "Hypoxia", oxygen s)
...
# !! BEFORE RUNNING !!:
  ...
  #' !!! Manually update for now - copy from 03_AK_climate_energetics on the PC to the data folder here
