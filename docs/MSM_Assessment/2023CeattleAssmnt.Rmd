...
```{r maincalcs, echo=FALSE, eval = F, include=F, message=FALSE, results='hide',fig.show = 'hide'}
    ...
    get_val <- function(kind = "SSB",m0 = m_0_1_3,m2 = m_2_1_3,spnames =splist[1:3]){
      #"meta"  "years" "SSB"   "rec"   "catch" "Frate"
        ...
        return(out)
         ...
         pp_SSB <- ggplot()+
           ...
           scale_color_viridis_d(begin=.1,end=.7)+theme_minimal()+
      ...
      if(!file.exists("Figs")) dir.create("Figs")
        ...
        get_SSBESM <- function(s =1, 
          ...
          colnames(tt0) <- c(colnames(FUT_set0$ts$meta),colnames(FUT_set0$ts$SSB))
          
          tt2 <- data.frame(FUT_set2$ts$meta,FUT2$ts$SSB)
          colnames(tt2) <- c(colnames(FUT_set2$ts$meta),colnames(FUT_set2$ts$SSB))
       ...
       gg<-viridis::viridis(3)[1]
        p_delta <- ggplot(dlt_SSB%>%filter(MSMmode==0,year>2025,simlab!="Persistence",simlab!="all"))+
          geom_col(aes(x=simlab,y=mn_deltaP*100,fill =simlab))+
          geom_point(aes(x=simlab,y=mn_deltaP*100),color=gg)+
          geom_errorbar(aes(x=simlab, 
                            ymin=lower_deltaP*100,
                            ymax=upper_deltaP*100),size=.9,color=gg, width = 0.6)+
          facet_grid(species~year)+
          scale_fill_viridis_d(begin =.1,end=.8,direction=-1,name="Climate scenario")+ theme_minimal()+
      ...
      # getProb  <- function(mn,sd,lim,gt=TRUE){
      #    tt   <- rnorm(1e4,mn,sd)
      #    tt2  <- density(tt)
      #   if(gt){
  ...
  cor_table_0       <- data.frame(rbind(
      ...
      # fishA=c(
      #   cor( as.numeric(mod["fsh_age_obs_1"][[1]]),as.numeric(mod["fsh_age_hat_1"][[1]]) ),
      #   cor( as.numeric(mod["fsh_age_obs_2"][[1]]),as.numeric(mod["fsh_age_hat_1"][[1]]) ),
      #   cor( as.numeric(mod["fsh_age_obs_2"][[1]]),as.numeric(mod["fsh_age_hat_1"][[1]]) )),
      Cnum=c(
    ...
    cor_table_2   <- data.frame(rbind(
      ...
      # fishA=c(
      #   cor( as.numeric(mod["fsh_age_obs_1"][[1]]),as.numeric(mod["fsh_age_hat_1"][[1]]
      #                                                         [match(mod["yrs_fsh_comp_1"][[1]][1,],1979-1+1:nyrs)]) ),
      #   cor( as.numeric(mod["fsh_age_obs_2"][[1]]),as.numeric(mod["fsh_age_hat_1"][[1]]) ),
      #   cor( as.numeric(mod["fsh_age_obs_2"][[1]]),as.numeric(mod["fsh_age_hat_1"][[1]]) )),
      Cnum=c(
        cor( as.numeric(mod["tc_biom_obs_1"][[1]]),as.numeric(mod["obs_catch_hat_1"][[1]]) ),
       ...
       getC_old<-function(s,m=2,y=nyrs,max=F){
         ...
         getC <- function(s,m=2,y=nyrs,max=F,scenIN = 1:nscens){
          ...
          #     if(m==2)  tmpd <-B0_list[[2]]
          #     tmpd<- tmpd%>%filter(sp==s,Scen%in%scenIN)%>%
          #       select(sp,Scen,Fabc)%>%ungroup()   
          #       FThisYr <-  mean(tmpd$Fabc, na.rm=T)
          #      
          # }
           eval(parse(text=paste0( "Fage<-(mod$fsh_sel_",s,"[1,1:nages[",s,"]]*FThisYr)" ) ))
            eval(parse(text=paste0( "M<-mod$M1_",s,"[1,1:nages[",s,"]]+mod$M2_",s,"[nyrs,1:nages[",s,"]]")))
    ...
    rec_0 <- load_recruitment_results(pth = pth_0)
     ...
     load( file.path(run_fl,"Figs/colIT.Rdata"))
      ...
      get_AIC_smry <- function (rec_in,sub_varIN, modeIN ,splistIN=factor(splist,levels = splist)){
        ...
        rec_in[[1]][[3]]%>%mutate(Species=splistIN[3],mode = modeIN))%>%
          filter(topSet=="**o")%>%
          select(Species,mode, model,R2,aicc,deltaAIC,cumlAIC,rank)%>%
        ...
        return(AIC%>%select(-model)%>%relocate(model=model2, Type, R2,aicc,deltaAIC,cumlAIC,Species,mode,rank))
      }
     ...
     aic_n <- AIC%>%group_by(Species, mode)%>%summarize(n = length(model))%>%cast(Species~mode,value="n")
      ...
      AIC_0_sub <- AIC_0%>%filter(rank<4)
       AIC_2_sub <- AIC_2%>%filter(rank<4)
...
```{r startup, echo=FALSE, include=T,eval=TRUE, message=FALSE, results='hide',fig.show = 'hide'}
  #setwd("/Volumes/LaCie/GitHub_cloud/CEATTLE_projects/2023_Multispp_assessment/docs/MSM_Assessment")
 thisYr <- as.numeric(format(Sys.time(), "%Y"))
 today  <- format(Sys.time(), "%b %d, %Y")
 suppressWarnings(source("../Assessment_setup.R"))
...
**Suggested citation:** _Holsman, K. K., J. Ianelli,  K. Shotwell, S. Barbeaux, K. Aydin, G. Adams, K. Kearney, K. Shotwell (2023) Climate-enhanced multispecies stock assessment for walleye pollock, Pacific cod, and arrowtooth flounder in the eastern Bering Sea. North Pacific Fishery Management Council, Anchorage, AK._
...
Pollock age 1 recruitment estimates for `r thisYr` are `r round(deltaR[1,4]*100)`% `r verbal_trend(type=3,x=deltaR[1,4]*100)` the 1979-2015 average and estimated recruitment has `r verbal_trend(deltaR[1,1]*100)` in `r thisYr` relative to `r thisYr-1` ( note that the most recent estimates have the highest uncertainty). Pacific cod age 1 recruitment in the EBS remains `r (round(deltaR[2,4]*100))`% `r verbal_trend(type=4,deltaR[2,4]*100)` the 1979-2015 average, and is similar in `r thisYr` to the record low recruitment estimated for `r years[which(ceattle_2$rec_dev[2,]==min(ceattle_2$rec_dev[2,]))]`.Estimates of arrowtooth flounder age 1 recruitment `r verbal_trend(type=2,deltaR[3,1]*100)`d relative to `r thisYr-1` but is `r (round(deltaR[3,4]*100))`% `r verbal_trend(type=3,deltaR[3,4]*100)` the 1979-2015 average.
...
knitr::kable(AIC_0_sub%>%select(-rank), row.names = FALSE, padding = 2, 
      align = "lllcccllc", caption="<left>a) Single-species</left>")
# %>%
...
# library(knitr)
# library(kableExtra)
knitr::kable(AIC_2_sub%>%select(-rank), row.names = FALSE, padding = 2, 
      align = "lllcccllc", caption="<left>b) Multi-species</left>")
